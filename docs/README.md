...
| [`tdd.md`](tdd.md) | **How a Go program satisfies all of it.** Stack, architecture, the `Plan` model, derivation pipeline, config parsing, command surface, safety stances, testing | Approved |
| [`tech-decision-log.md`](tech-decision-log.md) | **Why the technical design says that.** `T1`–`T30`, same rules as the D-log, separate namespace | Approved |
...
anything that looks arbitrary. It usually is not.
...
no ordering-as-plan; it references `M1`–`M4` purely as scope anchors ("this applies from M2
...
There is no repository-root `README.md` yet; it arrives with M0
([`roadmap.md`](roadmap.md) §2), along with the first Go code. The predecessor's `README.rst` is
not carried into this repository — `design.md` supersedes its "Primary Goals" and "Future Ideas"
sections, and [`project-assessment-2026-08.md`](project-assessment-2026-08.md) records what was
there.
