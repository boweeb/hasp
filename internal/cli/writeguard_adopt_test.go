...
// TestWriteNothingOutsideKeyDir_AdoptReleaseProfile extends the write-nothing guard to `adopt
// profile` and `release profile`: every write either command makes must land inside --key-dir.
func TestWriteNothingOutsideKeyDir_AdoptReleaseProfile(t *testing.T) {
	home := t.TempDir()
	keyDir := filepath.Join(home, ".ssh")
	profileDir := filepath.Join(keyDir, "work")
	if err := os.MkdirAll(profileDir, 0o700); err != nil {
	...
	run("adopt", "profile", "work", "--yes", "--key-dir", keyDir)
	run("release", "profile", "work", "--yes", "--key-dir", keyDir)
...
// TestWriteNothingOutsideKeyDir_AdoptProfile_RejectsProfileTraversal mirrors the adopt-key
// traversal regression test: a malicious profile-path argument must be rejected before anything is
// written, anywhere.
func TestWriteNothingOutsideKeyDir_AdoptProfile_RejectsProfileTraversal(t *testing.T) {
	home := t.TempDir()
	keyDir := filepath.Join(home, ".ssh")
	if err := os.MkdirAll(keyDir, 0o700); err != nil {
		t.Fatal(err)
	...
	root := cli.NewRootCmd()
	root.SetArgs([]string{"adopt", "profile", "work/../../../tmp", "--yes", "--key-dir", keyDir})
	root.SetOut(&bytes.Buffer{})
	root.SetErr(&bytes.Buffer{})
	err := root.Execute()
	if err == nil {
		t.Fatal("hasp adopt profile work/../../../tmp: want an error, got nil")
	}
	if got := cli.ExitCode(err); got != 2 {
		t.Errorf("ExitCode(%v) = %d, want 2 (ErrUsage)", err, got)
	}
	...
	if _, statErr := os.Stat(filepath.Join(evilDir, ".hasp")); statErr == nil {
		t.Errorf("write-nothing-outside-key-dir guard: %s was created outside --key-dir by a traversal profile", filepath.Join(evilDir, ".hasp"))
	}
}
